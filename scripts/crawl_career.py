#!/usr/bin/env python3
"""Crawl www.njit.edu/careerservices/ (whole subtree, path-scoped DFS) -> KB prose + KG staff.

Org 18 is Career Development Services. The roster lives on /careerservices/contact-us (SAME shape as
Dean of Students: <header> / Surname,Given / title / View Profile; no per-person email). People are
URL-gated to that page; some staff are cross-listed under two teams (deduped by name). Dry-run by
default; --commit takes a hardened backup first, then writes via career_crawl.ingest_career. DB-only
change -> no bot restart; run embed_all afterwards (or pass --embed).

Gated:  cp gsa_gateway.db /tmp/dev.db
        python scripts/crawl_career.py --db /tmp/dev.db [--commit]   # dev
        python scripts/crawl_career.py --commit --embed              # live
Then: python scripts/_career_cleanup_migrate.py [--commit] ; python scripts/_career_alias_migrate.py [--commit]
Spec: docs/superpowers/specs/2026-06-24-career-crawl-design.md
"""
...
from v2.core.ingestion import career_crawl
from v2.core.ingestion.web_crawler import make_fetcher

ENTRY_POINTS = ["https://www.njit.edu/careerservices/"]
...
def main(argv=None) -> int:
    ...
    for seed in seeds:
        print(f"\n# {seed}")
        res = career_crawl.extract_entry(seed, fetch, budget=400)
        results.append(res)
    ...
    hardened_backup(args.db, "pre-career-crawl")
    conn = get_connection(args.db)
    summary = {"staff": 0, "prose_inserted": 0, "prose_updated": 0, "prose_unchanged": 0}
    for res in results:
        r = career_crawl.ingest_career(conn, res)
        for k in summary:
            summary[k] += r.get(k, 0)
    conn.commit()
