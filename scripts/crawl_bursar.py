#!/usr/bin/env python3
"""Crawl www.njit.edu/bursar/ (whole subtree, path-scoped DFS) -> KB prose (+ KG staff if any).

Bursar is a prose-only office (no published roster), so staff is normally 0. Dry-run by default
(crawls + prints the manifest, no DB writes). --commit takes a hardened backup first, then writes
via bursar_crawl.ingest_bursar and commits. DB-only change -> no bot restart; run embed_all
afterwards (or pass --embed).

Gated workflow:  cp gsa_gateway.db /tmp/dev.db
                 python scripts/crawl_bursar.py --db /tmp/dev.db            # dev dry-run
                 python scripts/crawl_bursar.py --db /tmp/dev.db --commit   # dev write, inspect
                 python scripts/crawl_bursar.py --commit --embed            # live

After the live crawl verifies, run the SEPARATE clean-replace migration to retire the old
pre-crawler rows: python scripts/_bursar_cleanup_migrate.py (dry-run; --commit).
...
from v2.core.ingestion import bursar_crawl
from v2.core.ingestion.web_crawler import make_fetcher

# Bursar is one office at one path-prefix; a single homepage seed walks the whole subtree.
ENTRY_POINTS = ["https://www.njit.edu/bursar/"]
...
def main(argv=None) -> int:
    ...
    for seed in seeds:
        print(f"\n# {seed}")
        res = bursar_crawl.extract_entry(seed, fetch, budget=400)   # whole subtree backstop
        results.append(res)
    ...
    hardened_backup(args.db, "pre-bursar-crawl")
    conn = get_connection(args.db)
    summary = {"staff": 0, "prose_inserted": 0, "prose_updated": 0, "prose_unchanged": 0}
    for res in results:
        r = bursar_crawl.ingest_bursar(conn, res)
        for k in summary:
            summary[k] += r.get(k, 0)
    conn.commit()
    ...
    if args.embed:
        ...
        print("next: python v2/scripts/embed_all.py   # embed the new Bursar chunks")
    return 0
