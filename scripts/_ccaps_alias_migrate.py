"""One-off gated migration: add name aliases to the C-CAPS / Counseling org (id 19).

Org 16 slug is `counseling` and name "Counseling Center (C-CAPS)". This adds the acronym + name
phrasings students use ("c-caps", "ccaps", "counseling center", …) so those
...
SLUG = "counseling"
NEW_ALIASES = ["c-caps", "ccaps", "counseling center", "counseling and psychological services", "counseling"]
...
def run(db: str, commit: bool) -> int:
    ...
    try:
        ...
        hardened_backup(db, label="ccaps-alias")
        meta["aliases"] = merged
