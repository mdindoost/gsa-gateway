#!/usr/bin/env python3
"""Crawl www.njit.edu/admissions/ (whole subtree, path-scoped DFS) -> KB prose + KG staff.

Org 21 is the OFFICE OF UNIVERSITY ADMISSIONS (owner: full crawl, site = source of truth). The
office team (~26) lives on /admissions/contact-admissions; people are URL-gated to that page, so the
grad-advisors directory (faculty cross-listed as advisors) and every other page are prose-only.
Dry-run by default (crawls + prints the manifest, no DB writes). --commit takes a hardened backup
first, then writes via admissions_crawl.ingest_admissions and commits. DB-only change -> no bot
restart; run embed_all afterwards (or pass --embed).

Gated workflow:  cp gsa_gateway.db /tmp/dev.db
                 python scripts/crawl_admissions.py --db /tmp/dev.db            # dev dry-run
                 python scripts/crawl_admissions.py --db /tmp/dev.db --commit   # dev write, inspect
                 python scripts/crawl_admissions.py --commit --embed            # live

After the live crawl verifies, run the SEPARATE clean-replace migration to retire the old
pre-crawler rows: python scripts/_admissions_cleanup_migrate.py (dry-run; --commit), then add the
org aliases: python scripts/_admissions_alias_migrate.py (dry-run; --commit).

Spec: docs/superpowers/specs/2026-06-24-admissions-crawl-design.md
"""
...
from v2.core.ingestion import admissions_crawl
from v2.core.ingestion.web_crawler import make_fetcher

ENTRY_POINTS = ["https://www.njit.edu/admissions/"]
...
def main(argv=None) -> int:
    ...
    for seed in seeds:
        print(f"\n# {seed}")
        res = admissions_crawl.extract_entry(seed, fetch, budget=400)
        results.append(res)
    ...
    hardened_backup(args.db, "pre-admissions-crawl")
    conn = get_connection(args.db)
    summary = {"staff": 0, "prose_inserted": 0, "prose_updated": 0, "prose_unchanged": 0}
    for res in results:
        r = admissions_crawl.ingest_admissions(conn, res)
        for k in summary:
            summary[k] += r.get(k, 0)
    conn.commit()
    ...
    if args.embed:
        ...
        print("next: python v2/scripts/embed_all.py   # embed the new Admissions chunks")
    return 0
