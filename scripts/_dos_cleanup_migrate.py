#!/usr/bin/env python3
"""Clean-replace the pre-crawler Dean of Students KB rows + supersede the manual people (gated migration).

Run ONLY after crawl_dos.py --commit has written + verified the new crawler rows.
...
Spec: docs/superpowers/specs/2026-06-24-dos-crawl-design.md (G7)
"""
...
_DOS_URL = re.compile(r"njit\.edu/dos(/|$)", re.I)


def _dos_org_node(conn):
    oid = conn.execute("SELECT id FROM organizations WHERE slug=?", (DOS_SLUG,)).fetchone()[0]
...
def select_people(conn) -> tuple[list[dict], list[dict]]:
    ...
    oid, onode = _dos_org_node(conn)
    if onode is None:
        return [], []
    crawler_names = {_norm_name(r[0]) for r in conn.execute(
        "SELECT name FROM nodes WHERE type='Person' AND key LIKE 'crawler/dean-of-students/%'") if r[0]}
    retire, review = [], []
...
def _is_dos_site_url(url) -> bool:
    return bool(url) and bool(_DOS_URL.search(url))
...
def select_retire(conn) -> list[dict]:
    ...
    for rid, cb, title, url in rows:
        ...
        elif cb == "dashboard" and has_crawler and _is_dos_site_url(url):
            retire.append({"id": rid, "created_by": cb, "title": title,
                           "source_url": url, "reason": "superseded-by-crawler"})
    return retire
...
def main(argv=None) -> int:
    ...
    _, onode = _dos_org_node(conn)
    hardened_backup(args.db, "pre-dos-cleanup")
