#!/usr/bin/env python3
"""Crawl ist.njit.edu (whole site, host-scoped DFS) -> KG staff + KB prose.
...
Gated workflow:  cp gsa_gateway.db /tmp/dev.db
                 ...
                 python scripts/crawl_ist.py --commit --embed            # live
...
def main(argv=None) -> int:
    ...
    for seed in seeds:
        ...
        for p in sorted(res.prose, key=lambda x: x.source_url):
            ...
            print(f"    - {p.title[:44]:44} | {len(p.content):5}ch | "
                  f"{p.source_url.replace('https://ist.njit.edu', '')}{fig}")
    ...
    if args.embed:
        ...
        print("next: python v2/scripts/embed_all.py   # embed the new IST chunks")
    return 0
