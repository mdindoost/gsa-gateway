#!/usr/bin/env python3
"""Crawl www.njit.edu/dos/ (whole subtree, path-scoped DFS) -> KB prose + KG staff.

Org 20 is the Office of the DEAN OF STUDENTS. The 7-person roster lives on /dos/contact.php
('View Profile'-terminated blocks: header / Surname,Given / title / View Profile; no per-person
email). People are URL-gated to that page; every other page is prose-only. Dry-run by default;
--commit takes a hardened backup first, then writes via dos_crawl.ingest_dos and commits. DB-only
change -> no bot restart; run embed_all afterwards (or pass --embed).

Gated workflow:  cp gsa_gateway.db /tmp/dev.db
                 python scripts/crawl_dos.py --db /tmp/dev.db            # dev dry-run
                 python scripts/crawl_dos.py --db /tmp/dev.db --commit   # dev write, inspect
                 python scripts/crawl_dos.py --commit --embed            # live

After the live crawl verifies, run the SEPARATE clean-replace migration:
python scripts/_dos_cleanup_migrate.py (dry-run; --commit), then add org aliases:
python scripts/_dos_alias_migrate.py (dry-run; --commit).

Spec: docs/superpowers/specs/2026-06-24-dos-crawl-design.md
"""
...
from v2.core.ingestion import dos_crawl
from v2.core.ingestion.web_crawler import make_fetcher

ENTRY_POINTS = ["https://www.njit.edu/dos/"]
...
def main(argv=None) -> int:
    ...
    for seed in seeds:
        print(f"\n# {seed}")
        res = dos_crawl.extract_entry(seed, fetch, budget=400)
        results.append(res)
    ...
    if args.embed:
        ...
        print("next: python v2/scripts/embed_all.py   # embed the new Dean of Students chunks")
    return 0
