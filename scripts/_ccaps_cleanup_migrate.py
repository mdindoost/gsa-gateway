#!/usr/bin/env python3
"""Clean-replace the pre-crawler C-CAPS KB rows + supersede the pre-crawler people (SEPARATE gated migration).

Run ONLY after crawl_ccaps.py --commit has written + verified the new crawler rows.
Retires: every superseded njit-crawl/migration KB row, and every dashboard KB row that points at a
www.njit.edu/counseling WEB page (any URL alias, ANY type). KEEPS: any dashboard row whose source_url is
NOT a live /counseling page (genuinely manual) and every crawler row.

PEOPLE: C-CAPS had pre-crawler people from BOTH njit-crawl (e.g. Rebecca Wolk) and possibly dashboard.
...
Spec: docs/superpowers/specs/2026-06-24-ccaps-crawl-design.md (G7)
"""
...
# Anchored: njit.edu/counseling followed by '/' or end-of-string — covers /counseling, /counseling/contact,
# /counseling/optional-practical-training; never a hypothetical /counseling-foo path. Forward-safe.
_CCAPS_URL = re.compile(r"njit\.edu/counseling(/|$)", re.I)


def _ccaps_org_node(conn):
    oid = conn.execute("SELECT id FROM organizations WHERE slug=?", (CCAPS_SLUG,)).fetchone()[0]
...
def select_people(conn) -> tuple[list[dict], list[dict]]:
    ...
    oid, onode = _ccaps_org_node(conn)
    if onode is None:
        return [], []
    crawler_emails, crawler_names = set(), set()
    for nm, attrs in conn.execute(
        "SELECT name, attrs FROM nodes WHERE type='Person' AND key LIKE 'crawler/counseling/%'"):
        if nm:
...
def _is_ccaps_site_url(url) -> bool:
    return bool(url) and bool(_CCAPS_URL.search(url))
...
def select_retire(conn) -> list[dict]:
    ...
    for rid, cb, title, url in rows:
        ...
        elif cb == "dashboard" and has_crawler and _is_ccaps_site_url(url):
            retire.append({"id": rid, "created_by": cb, "title": title,
                           "source_url": url, "reason": "superseded-by-crawler"})
    return retire
...
def main(argv=None) -> int:
    ...
    _, onode = _ccaps_org_node(conn)
    hardened_backup(args.db, "pre-ccaps-cleanup")
