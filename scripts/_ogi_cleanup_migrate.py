#!/usr/bin/env python3
"""Clean-replace the pre-crawler OGI KB rows + supersede the pre-crawler people (SEPARATE gated migration).

Run ONLY after crawl_ogi.py --commit has written + verified the new crawler rows.
Retires: every superseded njit-crawl/migration KB row, and every dashboard KB row that points at a
www.njit.edu/global WEB page (any URL alias, ANY type). KEEPS: any dashboard row whose source_url is
NOT a live /global page (genuinely manual) and every crawler row.

PEOPLE: OGI had pre-crawler people from BOTH njit-crawl (e.g. Rebecca Wolk) and possibly dashboard.
...
Spec: docs/superpowers/specs/2026-06-24-ogi-crawl-design.md (G7)
"""
...
# Anchored: njit.edu/global followed by '/' or end-of-string — covers /global, /global/contact,
# /global/optional-practical-training; never a hypothetical /global-foo path. Forward-safe.
_OGI_URL = re.compile(r"njit\.edu/global(/|$)", re.I)


def _ogi_org_node(conn):
    oid = conn.execute("SELECT id FROM organizations WHERE slug=?", (OGI_SLUG,)).fetchone()[0]
...
def select_people(conn) -> tuple[list[dict], list[dict]]:
    ...
    oid, onode = _ogi_org_node(conn)
    if onode is None:
        return [], []
    crawler_emails, crawler_names = set(), set()
    for nm, attrs in conn.execute(
        "SELECT name, attrs FROM nodes WHERE type='Person' AND key LIKE 'crawler/ogi/%'"):
        if nm:
...
def _is_ogi_site_url(url) -> bool:
    return bool(url) and bool(_OGI_URL.search(url))
...
def main(argv=None) -> int:
    ...
    _, onode = _ogi_org_node(conn)
    hardened_backup(args.db, "pre-ogi-cleanup")
