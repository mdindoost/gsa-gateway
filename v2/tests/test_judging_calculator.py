...
def test_progress_all_scored(seeded):
    conn, eid = seeded
...
def test_export_csv_contains_names(seeded):
    conn, eid = seeded
    csv = export_csv(conn, eid)
    assert "Jane Smith" in csv
    assert "Ali Hassan" in csv
    assert "Maria Garcia" in csv
