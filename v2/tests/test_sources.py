...
def test_rejects_unknown_org(conn):
    with pytest.raises(EnqueueError, match="does not exist"):
        enqueue_post(conn, conn, PostDraft(org_id=999, content="x", type="broadcast"))
...
def test_dedup_returns_existing_id(conn):
    ...
    first = enqueue_post(conn, conn, d)
    second = enqueue_post(conn, conn, d)
    assert first == second
...
def test_dedup_on_source_id(conn):
    ...
    d2 = PostDraft(org_id=2, content="v2 different content", type="broadcast",
                   source_type="event", source_id=7)
    first = enqueue_post(conn, conn, d1)
    second = enqueue_post(conn, conn, d2)   # same source_id -> dedup hit despite different content
    assert first == second
...
def test_runner_enqueues_polled_drafts(conn):
    src = _FakeSource([PostDraft(org_id=2, content="from source", type="broadcast",
                                 source_type="fake")])
    runner = SourceRunner(conn, conn, src)
...
def test_runner_isolates_a_throwing_source(conn):
    runner = SourceRunner(conn, conn, _FakeSource(RuntimeError("boom")))
...
def test_runner_skips_invalid_drafts_but_keeps_good_ones(conn):
    ...
    runner = SourceRunner(conn, conn, _FakeSource(drafts))
    n = _run(runner.run_once())
    assert n == 1
...
def test_runner_enforces_flood_cap(conn):
    ...
    runner = SourceRunner(conn, conn, _FakeSource(many))
    n = _run(runner.run_once())
...
def test_runner_start_polls_then_stops_cleanly(conn):
    src = _FakeSource([])
    runner = SourceRunner(conn, conn, src, interval=0)
