"""Career Development Services crawler (Crawling 2.1, office rollout #7, the LAST office).

A copy of the Dean of Students crawler (SAME roster shape) adapted for www.njit.edu/careerservices/:
the contact page /careerservices/contact-us renders each person as a block ENDING with 'View Profile'
(<section header> / Surname,Given / title / View Profile), no per-person email. parse_roster + the
'Surname, Given' name detector + section-header persistence are reused UNCHANGED from dos_crawl (so
this office adds NO novel parsing — no separate review). People URL-gated to /careerservices/contact-us.
Some staff are cross-listed under two teams (e.g. Lanzot, Sims) -> deduped by name (kept once).
Brings data ONLY (2026-06-23 hard line).
"""
...
CAREER_NAME = "Career Development Services"
...
_CONTACT_PATH = "/careerservices/contact-us"
...
def parse_roster(text: str) -> tuple[list[StaffRecord], list[str]]:
    ...
    for ln in lines:
        ...
        title = " ".join(titles)
        if name in seen:
            # legitimate on this page — several staff are cross-listed under two teams (e.g. Lanzot,
            # Sims). Deduped to one record; counted so the recount below isn't a false alarm.
            dups += 1
            warnings.append(f"duplicate roster name (cross-listed under 2 teams), kept first: {name!r}")
        else:
            seen.add(name)
            records.append(StaffRecord(name=name, title=title, phone="", email="",
                                       unit=section or CAREER_NAME, titles=(title,)))
        buf = []

    # recount accounts for cross-listed people (a person listed under N teams = N 'View Profile's)
    if len(records) + dups != vp_count:
        warnings.append(
            f"parsed {len(records)} people (+{dups} cross-listings) but found {vp_count} 'View "
            f"Profile' markers — possible roster-structure change; review before trusting the roster")
...
def ingest_career(conn, result: EntryResult, source: str = "crawler") -> dict:
    """Write an EntryResult under the EXISTING 'career-development' org (id 18, under njit): staff ->
    ...
    org_id = ensure_org(conn, CAREER_SLUG, CAREER_NAME, parent_slug="njit", type="office")
    sync_org_nodes(conn)

    for s in result.staff:
        key = f"{source}/{CAREER_SLUG}/{_slug(s.name)}"
        pid = project_appointment(
            conn, person_key=key, name=s.name, org_id=org_id, category="staff",
            titles=list(s.titles) or [s.title], source_section=(s.unit or CAREER_NAME), source=source)
