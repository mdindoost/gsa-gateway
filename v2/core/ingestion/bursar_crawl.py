"""Office of the Bursar / Student Accounts crawler.

Bursar-specific extractor — a copy of the Graduate Studies (GSO) crawler adapted for
www.njit.edu/bursar/: the same path-prefix DFS and verbatim prose ingest. Reuses the
...
Bursar publishes NO named personnel (its contact-us page lists office-level departmental
mailboxes only), so the inherited roster parser returns [] and the office is prose-only:
0 KG Person nodes is the correct, honest outcome. A function-email GUARD (below) makes that
guarantee robust to a future stray 'personnel' anchor next to a departmental mailbox — a
departmental address (bursar@/payment@/…) is never fabricated into a Person.

Spec: docs/superpowers/specs/2026-06-24-bursar-crawl-design.md
"""
...
BURSAR_SLUG = "bursar"
BURSAR_NAME = "Office of the Bursar / Student Accounts"
...
_FUNCTION_MAILBOXES = frozenset({
    "bursar", "healthinsurance", "thirdparty", "payment", "collections",
    "info", "contact", "admissions", "help",
})
...
def ingest_bursar(conn, result: EntryResult, source: str = "crawler") -> dict:
    """Write an EntryResult under the EXISTING 'bursar' org (id 17, under njit):
    ...
    org_id = ensure_org(conn, BURSAR_SLUG, BURSAR_NAME, parent_slug="njit", type="office")
    sync_org_nodes(conn)

    for s in result.staff:
        key = f"{source}/{BURSAR_SLUG}/{_slug(s.name)}"
