"""Office of Financial Aid crawler (Crawling 2.1, office rollout #4).

A copy of the Bursar prose-only crawler adapted for www.njit.edu/financialaid/: same path-prefix
DFS + verbatim prose ingest reusing the web_crawler spine. Brings data ONLY (2026-06-23 hard line).

Financial Aid publishes NO named-staff roster (its contact-us page lists only the departmental
mailbox finaid@njit.edu + phone; a few individual contacts — e.g. a veterans certifying official —
appear INLINE in prose, captured verbatim in KB). So the inherited 'personnel'-anchored roster
parser returns [] and the office is prose-only: 0 KG Person nodes is the correct, honest outcome.
The function-email GUARD keeps that robust (finaid@/eop@/honors@/… never fabricated into a Person).
Funding content is high-stakes → the serve-time heads-up covers it; the crawler just brings data.

Spec: docs/superpowers/specs/2026-06-24-financialaid-crawl-design.md
"""
...
FINAID_NAME = "Office of Financial Aid"

_EMAIL = re.compile(r"^[A-Za-z0-9._%+'-]+@njit\.edu$", re.I)
_PHONE = re.compile(r"\b(\d{3}-\d{3}-\d{4})\b")
_ROSTER_ANCHOR = "personnel"            # a contact-page Personnel block header (absent on Bursar today)
...
_TITLE_CUES = ("provost", "dean", "director", "coordinator", "manager", "assistant",
               ...
               "advisor", "adviser", "counselor", "counsel", "vice president", "office",
               "chair", "analyst", "secretary", "representative", "generalist")
# Anti-fab guard (RAG-review B1): departmental FUNCTION mailboxes are NOT people. If a future
# Bursar page introduces the 'personnel' anchor next to one of these, the parser must not
# fabricate a Person from an office label + a function address — it warns instead.
_FUNCTION_MAILBOXES = frozenset({
    "finaid", "financialaid", "eop", "honors", "admissions", "registrar", "global",
    "njitvso", "client_services", "info", "contact", "help", "sfas",
})
...
class EntryResult:
    ...
    warnings: list[str] = field(default_factory=list)   # unparseable / function-mailbox rows (flag)
...
def extract_entry(seed: str, fetch, max_depth: int = 4, budget: int = 300) -> EntryResult:
    """Crawl the Bursar entry point and extract: every page -> prose (KB), empty shells ->
    skipped (flagged). Bursar has no roster, so staff is normally []. COVERAGE RULE: a contact
    page is NOT dropped from prose — its office-contact prose (emails/phone/hours) is kept too.
    Prose deduped by content hash (collapsing .php / clean-URL aliases), keeping the cleanest
    URL. Brings data only; no DB writes."""
...
def classify_page(html: str) -> str:
    """Decide how a page should be handled: ``staff-roster`` (people → KG; won't fire for
    Bursar — no personnel), ``prose`` (content → KB), or ``skip-empty`` (no readable main
    content → flag, never store). ``parse_roster`` returns ``(records, warnings)``."""
...
def _is_title(line: str) -> bool:
    ...
    the KNOWN BOUNDARY of the parser: a real staffer whose title carries NO cue word won't be
    recognized and that record surfaces as a WARNING for the owner to hand-fix — anti-fab: warn,
    never fabricate a name/title, never silent-drop."""
    low = line.lower()
    return "(" in line or any(c in low for c in _TITLE_CUES)


def parse_roster(text: str) -> tuple[list[StaffRecord], list[str]]:
    """Parse a contact-page 'Personnel' block. Each person renders as
        [section header?] / Name / Title(+more titles) / bare-phone / email
    Phone-anchored: a (bare-phone, email) adjacent pair marks a record tail; the lines above
    the phone are name + title(s), optionally preceded by a section header. Anti-fab:
      - a chunk that can't yield (name, >=1 title, email) is a WARNING, never dropped/invented;
      - a DEPARTMENTAL function mailbox (bursar@/payment@/…) is NEVER a person — it warns
        instead of fabricating one from an office label (RAG-review B1 guard).
    Returns ([], []) for any non-Personnel page (anchor absent) so it falls through to prose —
    this is the normal Bursar case (its pages contain no 'personnel' anchor)."""
    low = text.lower()
    ...
    for ln in lines:
        if _EMAIL.match(ln):
            ...
            if email.split("@", 1)[0].lower() in _FUNCTION_MAILBOXES:
                warnings.append(f"function mailbox {email!r} — not a person (skipped)")
                chunk = []
                continue
...
def ingest_financialaid(conn, result: EntryResult, source: str = "crawler") -> dict:
    """Write an EntryResult under the EXISTING 'financialaid' org (id 28, under njit):
      - staff -> Person + has_role(category='staff') + published contact attrs (KG) — normally
        none for Bursar (prose-only office).
    ...
    org_id = ensure_org(conn, FINAID_SLUG, FINAID_NAME, parent_slug="njit", type="office")
    sync_org_nodes(conn)

    for s in result.staff:
        key = f"{source}/{FINAID_SLUG}/{_slug(s.name)}"
