...
\begin{titlepage}
  ...
  {\large New Jersey Institute of Technology\\
   ...
   Discord Bot + Telegram Bot + Static Website}\\[2em]
  {\normalsize
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Discord Bot} — A Python bot that answers student questions using a
        ...
        digests.
  \item \textbf{Telegram Bot} (\texttt{@njit\_gsa\_bot}) — An independent bot that
        shares the same RAG brain as the Discord bot, providing free-form Q\&A and
        commands (\texttt{/events}, \texttt{/contact}, \texttt{/resources}) to students
        on Telegram.
...
\midrule
Discord framework & discord.py 2.x & Mature, async-native, slash command support \\
Telegram framework & python-telegram-bot 20.x & Async-native, matches asyncio stack \\
...
|   +-- main.py             # Discord entry point -- GSABot class, startup sequence
...
+-- run_telegram.py         # Telegram entry point (independent process)
+-- website/                # Static site
...
+-- gsa_gateway.log         # Discord bot log (auto-created)
+-- telegram_bot.log        # Telegram bot log (auto-created)
