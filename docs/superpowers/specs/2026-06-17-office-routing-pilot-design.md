...
Existing seed entries (OGI, Graduate Studies, Counseling, Library, Student Life, Wellness,
Inclusive Excellence) are **enriched** with a `Handles:` line; missing offices are **added**.
...
- **Entity capture (standing principle):** any **new NJIT office** encountered while drafting →
  an org node as above. Any **new relevant person** (an office director / named contact we find
  on the page) → `people_editor.add_or_edit_person(...)` → a Person node + `has_role` edge to the
  office + their contact, `source='dashboard'`, gated. **Bounded:** only entities we actually
  encounter for these offices — a *full* NJIT crawl remains a separate future effort (not this
  pilot). Nothing found is dropped.
...
- A frozen `{question → gold office slug/name}` map = the ~10 M intents **+ 3–5 paraphrases each**
  (~40–60 queries). e.g. "who do I contact about a billing hold", "where do I pay tuition",
  "my payment didn't go through who do I talk to" → **bursar**.
- Assert the correct office's chunk is in the reranked top-k for every query.
- **0 regressions:** a small guard set of existing non-office KB answers (travel award, CS GPA,
...
**Acceptance bar:** every M intent + paraphrase → correct office in top-k; 0 regressions; you've
verified the directory content.
...
- Create **`scripts/ingest_offices.py`** (new, gated — mirrors `ingest_office_docs.py`'s safety
  ...
  → `ensure_org(slug, name, parent_slug='njit', type='office')` → `upsert_doc_items(org_id,
  slug=<office slug>, ..., doc_type='contact')` → `sync_org_nodes`. Dry-run default,
  `hardened_backup` on `--commit`, then reminds to `embed_all.py`. (The folder→single-org
  `ingest_office_docs.py` doesn't fit because each office is its own org.)
